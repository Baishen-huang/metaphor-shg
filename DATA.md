...
> **⚠️ 评测口径修正（2026-09-25）**：本项目的检索类数字曾建立在两处有缺陷的
> 评测口径上，现已修复并量化：
> - **构造锚定**（"非构造金标"不成立）：632/632 的金标都含产出该问题的 chunk；
> - **候选池饱和**（median 7–8）：Hits@10 平凡为 1.0、随机 MRR 期望即达 0.37。
>
> 修复后（全局池 1,100 + 去锚定）：**anchored MRR 0.8956 / deanchor 0.2330**，
> 随机基线 0.0069 —— **锚定抬高 ≈ +0.66，但去锚定仍为随机的 34 倍**。
> 另：**L2/L3 覆盖率诚实口径为 82.4%**（报出口径 100%），低于 85% 门槛。
> 复现：`python -m metaphor_graph.evaluate_repaired`。
> 详见 `论文初稿.md` §4.1/§6.7 与 `experiments/实验结论汇总.md`。
