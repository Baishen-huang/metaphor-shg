...
| 构建（级联本体查表） | L2/L3 覆盖率 **100%**；构建成本比 LLM 聚类低一个数量级（~20 vs ~187 次/千边） |
| 双语资源 | **2,177 框架**，英文概念域对齐率 **100%** |
| 检索（改写型查询 n=632） | 语义超图通路召回 **1.000**；字面通路 **0.000**、触发词级联通路 **0.042** |
| 知识源消融（A6） | 专属隐喻级联 **0.783** vs 通用常识关联 **0.095**（跨域映射不可被常识替代） |
| 连贯文档扩展链 | 密度为独立短句语料的 **10×**；双段管线精度 **16.7% → 63.6%** |
| 复现护栏 | **116 项单元测试**；全部 LLM 调用落盘缓存，可零 API 费用重放 |
...
metaphor_graph/           核心实现
  ...
  test_metaphor_graph.py  116 项单元测试
release/                  可发布资源
